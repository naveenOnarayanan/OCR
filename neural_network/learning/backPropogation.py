...
class BackPropogationLearning:
    ...
    def propogate_error(self, output_delta):
        ...
        for i in range(self.network.layer_count - 1 , -1, -1):
            ...
            # print("Range", i)
            print("Inputs", self.network.layers[i].inputs[:-1])
            # print("Learning Rate", self.learning_rate)
            print("Delta", delta[-1])
            print(self.network.layers[i].inputs[:-1] * delta[-1])


            delta_weights = self.network.layers[i].inputs[:-1]*self.learning_rate*delta[-1]
            ...
            dd = self.network.layers[i].weights * d * delta[-1]
